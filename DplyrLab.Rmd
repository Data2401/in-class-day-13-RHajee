...
```{r}
my_flights$time_gained = (my_flights %>% mutate(time_gained = dep_delay - arr_delay))
my_flights
```
...
```{r}
my_flights %>% mutate(time_gained = dep_delay - arr_delay) %>%  arrange(time_gained) %>% select(time_gained, everything())
```
...
8. On average, did flights to Hobby gain or lose time?
```{r}
#mean(hou_flights$time_gained)
```
...
```{r}
my_flights %>% filter(origin == "JFK", dest == "SEA") %>% na.omit(cols = air_time, dep_time) %>% summarise(mean = mean(air_time), min = min(air_time), max = max(air_time))
...
```{r}
starwars %>% select(eye_color, height) %>% summarise()
